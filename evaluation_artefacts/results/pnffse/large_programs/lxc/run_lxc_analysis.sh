...
CONTAINER_NAME="lxc-analysis"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="${SCRIPT_DIR}/lxc-analysis-results/${TIMESTAMP}"

echo "=== ProveNFix lxc Analysis ==="
echo "Using existing spec_Lxc.c on main branch"
echo ""
...
echo ""
echo "[5/6] Downloading and configuring lxc (commit 72cc48f)..."
docker exec "$CONTAINER_NAME" bash -c '
    cd /home
    if [ ! -d "lxc" ]; then
...
echo ""
echo "[6/6] Copying spec_Lxc.c and running TempFix..."
docker exec "$CONTAINER_NAME" bash -c '
    cd /home/lxc
    cp /home/infer_TempFix/spec_Lxc.c spec.c
...
docker cp "${CONTAINER_NAME}:/home/lxc/spec.c" "${OUTPUT_DIR}/spec.c" 2>/dev/null || true
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    docker exec -it "$CONTAINER_NAME" bash -c "cd /home/lxc && bash"
fi
