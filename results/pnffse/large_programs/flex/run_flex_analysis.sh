...
CONTAINER_NAME="flex-analysis"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="${SCRIPT_DIR}/flex-analysis-results/${TIMESTAMP}"

echo "=== ProveNFix flex Analysis ==="
echo "Using spec_flex.c on main branch"
echo ""
...
# Download and configure flex
echo ""
echo "[5/6] Downloading and configuring flex (commit d3de49f)..."
docker exec "$CONTAINER_NAME" bash -c '
    cd /home
    if [ ! -d "flex" ]; then
...
echo ""
echo "[6/6] Copying spec_flex.c and running TempFix..."
docker exec "$CONTAINER_NAME" bash -c '
    cd /home/flex
    cp /home/infer_TempFix/spec_flex.c spec.c
...
docker cp "${CONTAINER_NAME}:/home/flex/spec.c" "${OUTPUT_DIR}/spec.c" 2>/dev/null || true
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    docker exec -it "$CONTAINER_NAME" bash -c "cd /home/flex && bash"
fi
